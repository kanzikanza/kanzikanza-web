...
public class AuthRelatedController {
        ...
        // .toList();
        //
        // var token = jwtIssuer.issue(principal.getUserIndex(),
        // principal.getUserEmail(), roles);
        ...
        // LoginResponse.builder().accessToken(token).build();
        ...
        public ResponseEntity<?> KakaoLoginServer(@RequestParam String code) {
                try {
                        ...
                        // Arrays.stream("".split(", "))
                        // .collect((Collectors.toList())));
                        // UserUniteDtos.LoginResponse loginResponse =
                        // UserUniteDtos.LoginResponse.builder().accessToken(MyAccesstoken).build();
                        // return ResponseEntity.ok().body(loginResponse);
                        // }
                        // else
                        // {
                        // var authentication = authenticationManager.authenticate(
                        // new UsernamePasswordAuthenticationToken(kakaoOpenIdToken.getSub(),
                        // kakaoOpenIdToken.getSub()));
                        // SecurityContextHolder.getContext().setAuthentication(authentication);
                        // var principal = (UserPrincipal) authentication.getPrincipal();
                        //
                        // var roles = principal.getAuthorities().stream()
                        // .map(GrantedAuthority::getAuthority)
                        // .toList();
                        // var MyAccesstoken = jwtIssuer.issue(principal.getUserIndex(),
                        // principal.getUserEmail(), roles);
